...
public class ConnectionService
{
    ...
    public CertificateInfoBean getCertificate(String host, Integer port)
    {
		...
		try
		{
		    ...
		    certificateInfoBean.setSigAlgName(chain[0].getSigAlgName());
		...
		catch (Exception exc)
		{
		    ...
		    certificateInfoBean.setResult(RESULT_CODE.ERROR);
		    certificateInfoBean.setErrorDescription(exc.getMessage());
		}
    ...
    private void setProxyConfiguration()
    {
		...
		if (((ConfigurationBean.HTTP_PROXY_USER != null) && (ConfigurationBean.HTTP_PROXY_USER.trim().length() > 0)) && ((ConfigurationBean.HTTP_PROXY_PASSWORD != null) && (ConfigurationBean.HTTP_PROXY_PASSWORD.trim().length() > 0)))
		{
			...
			{
			    return new PasswordAuthentication(ConfigurationBean.HTTP_PROXY_USER, ConfigurationBean.HTTP_PROXY_PASSWORD.toCharArray());
			}
		    });
		}
    }
